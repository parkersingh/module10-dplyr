...
manufacture <- select(vehicles, make)

# Use the `unique()` function to determine how many different car manufacturers
# are represented by the data set.
nrow(unique(manufacture))

# Filter the data set for vehicles manufactured in 1997
year.1997 <- filter(vehicles, year == 1997)
...
year.1997 <- arrange(year.1997, hwy)
...
year.1997 <- mutate(year.1997, average = (year.1997$cty + year.1997$hwy) / 2)
